...
struct Connection {
    ...
    E4Color color;               // Connection color
    ...
    int id; // Unique ID for identification
...
public:
    ...
    E4Color color; // Connection color (incoming branch)

    E4Color textColor = {0.0, 0.0, 0.0}; // Node text color
    ...
    Node(const std::string& t, E4Color c);
...
public:
    ...
    std::vector<std::shared_ptr<Connection>> connections;  // New arbitrary connections
    ...
    std::shared_ptr<Connection> hitTestConnection(double x, double y, double tolerance = 5.0);
    ...
    std::shared_ptr<Connection> addConnection(std::shared_ptr<Node> from, std::shared_ptr<Node> to);
